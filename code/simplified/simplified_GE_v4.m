...
while ((outer_diff > tol) && (outer_iter < maxiter))
    ...
    for t = 1:T
        ...
        % Given uncertainty and agg TFP, find the indexes of each indiviual
        % prod. level. (tricky to understand, contact linxi)
        ...
        aa1 = demand(i_qmax,t);
        aa2 = (nu/psi_n)^(nu/(1-nu))*sum(vec(dist_k(:,:,t).*(fine_grid.^aalpha*X'*zsim(t)).^(1/(1-nu))));
        csolve_v3 = @(c) c+psi*aa1-aa2*c^(nu/(nu-1)); % equation 4
        opts = optimset('Diagnostics','off', 'Display','off');
        Csim(t) = fsolve(csolve_v3,1,opts);
