...
while ((diff > tol) && (outer_iter < maxiter))
    ...
    disp(disp_text);
    disp_text = sprintf('log(q) = %d + %d * log(K) + %d * log(ssigmax) + %d*log(z)',pphi_qC,pphi_qK,pphi_qssigmax,pphi_qz);
    disp(disp_text);
    disp_text = sprintf('log(Kplus) = %d + %d * log(K) + %d * log(ssigmax)+ %d*log(z)',pphi_KC,pphi_KK,pphi_Kssigmax,pphi_Kz);
    disp(disp_text);
    disp_text = sprintf('log(C) = %d + %d * log(K) + %d * log(ssigmax)+ %d*log(z)',pphi_CC,pphi_CK,pphi_Cssigmax,pphi_Cz);
    disp(disp_text);
    disp_text = sprintf('KS Iter = %d, KS err = %d, Current VFI Iter = %d, err = %d',outer_iter,diff,iter,err);
    disp(disp_text);
    disp('===============================');
