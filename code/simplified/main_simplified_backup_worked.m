%% Housekeeping
% change ttau, or add quadratic adj cost?
...
profit = zeros(nk,ns); % Compute operation profit 
L = zeros(nk,ns);
for i_k = 1:nk
    for i_s = 1:ns
        [i_x,~] = ind2sub([nx 2],i_s);
        profit(i_k,i_s) = X(i_x)*k_grid(i_k)^aalpha;
    end
end
...
while ((diff > tol) && (outer_iter < maxiter))
    ...
    while err > tol
        for i_K = 1:nK
            ...
            for i_q = 1:nq
                q = q_grid(i_q);
                W_new(:,:,i_K,i_q) = profit - q*(repmat([k_grid(2:end);k_grid(end)],1,ns)-(1-ddelta)*repmat(k_grid,1,ns)) + bbeta*repmat(EV(nk,:),nk,1);
    ...
    X = [ones(T-burnin-1,1) log(Ksim(burnin+1:T-1))' log(ssigmax(ssigmaxsim(burnin+1:T-1)))'];
    ...
    diff = norm([pphi_KC,pphi_KK,pphi_Kssigmax,pphi_qC,pphi_qK,pphi_qssigmax]-[pphi_KC_new,pphi_KK_new,pphi_Kssigmax_new,pphi_qC_new,pphi_qK_new,pphi_qssigmax_new],Inf);
...
figure
plot(q_grid,revenue(:,T));
figure
plot(q_grid,demand(:,T));
toc
